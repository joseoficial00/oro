...
# ID del sticker que proporcionaste
...
def get_gold_price_ounce():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logging.error(f"Error en API: {response.status_code}")
            return None
...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Definición correcta de tiempo para evitar errores de NameError
    dt_now = datetime.now()
    fecha_str = dt_now.strftime("%d/%m/%2026")
    hora_str = dt_now.strftime("%I:%M:%S %p")
    ...
    if text == "📈 TASA EN TIEMPO REAL 💸":
        ...
        if price:
            msg = (
                ...
                f"⏱ <code>{fecha_str}  •  {hora_str}</code>\n\n"
        ...
        try:
            ...
            if price:
                ...
                total_compra = total_real * 0.90 # 10% menos para ganancia
                
                res = (
                    f"<tg-emoji emoji-id='{CUSTOM_EMOJI}'>✨</tg-emoji> <b>COTIZACIÓN</b>\n"
                    f"📅 <code>{fecha_str}</code>\n"
                    f"⏰ <code>{hora_str}</code>\n\n"
